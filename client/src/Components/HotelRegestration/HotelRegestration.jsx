...
const HotelRegestration = () => {
  ...
  const onSubmitHandler = async (e) => {
    ...
    try {
      ...
      if (data?.success) {
        ...
        const message = data?.message || "Couldn't register hotel";
        toast.error(message);
        console.log("Hotel registration failed:", message);
      }
    } catch (error) {
      console.log("Error from registration form:", error);
      const message =
        error?.response?.data?.message || error?.message || "Something went wrong";
      toast.error(`Hotel registration failed: ${message}`);
    }
  };
  ...
  return (
    ...
    >
      <div
        className="content relative bg-(--color-bg-card) shadow-(--shadow-card) flex justify-between items-center  lg:w-[65%] rounded-3xl lg:h-[80%] overflow-hidden transition-all duration-300"
        onClick={(e) => e.stopPropagation()} // منع إغلاق الـ modal عند الضغط داخل المحتوى
      >
        {/* Close Icon */}
        <X
          onClick={() => setShowHotelReg(false)}
        ...
        >
          ...
          <div className="hotelName w-full">
            <label htmlFor="name" className="text-(--color-text-secondary) text-sm">
              Hotel Name
            </label>
            <input
              value={name}
          ...
          <div className="phone w-full">
            <label htmlFor="phone" className="text-(--color-text-secondary) text-sm">
              Phone
            </label>
            <input
              value={contact}
          ...
          <div className="address w-full">
            <label htmlFor="address" className="text-(--color-text-secondary) text-sm">
              Address
            </label>
            <input
              value={address}
          ...
          <div className="city w-full">
            <label htmlFor="city" className="text-(--color-text-secondary) text-sm">
              City
            </label>
            <select
              value={city}
